...
public class GroundPlaneUI : MonoBehaviour
{
    ...
    const string CHAIR_PLACEMENT_HINT = "Tap to place product";
    const string CHAIR_CONTROLS_WITH_PINCH = "• Touch and drag to move product\n• Two fingers to rotate or pinch to scale";
    const string CHAIR_CONTROLS_WITHOUT_PINCH = "• Touch and drag to move product\n• Two fingers to rotate";
    ...
    bool SurfaceIndicatorVisibilityConditionsMet => mDevicePoseManager.TargetStatus.IsTrackedOrLimited() &&
                                                    ...
                                                    mTouchHandler.TouchCount == 0;

    void Start()
    {
        mDevicePoseManager = FindFirstObjectByType<DevicePoseManager>();
        mPlaneFinder = FindFirstObjectByType<PlaneFinderBehaviour>();
        mProductPlacement = FindFirstObjectByType<ProductPlacement>();
