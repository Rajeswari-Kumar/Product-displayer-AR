...
public class ProductPlacement : MonoBehaviour
{
    ...
    public Material[] mChairMaterials, mChairMaterialsTransparent;
    public Material mChairShadowMaterial, mChairShadowMaterialTransparent;
    Camera mMainCamera;
    string mFloorName;
    ...
    void Start()
    {
        ...
        mTouchHandler = FindFirstObjectByType<TouchHandler>();
    ...
    void Update()
    {
        ...
        if (mIsPlaced)
        {
            RotationIndicator.SetActive(mTouchHandler.TouchCount == 2);

            TranslationIndicator.SetActive((mTouchHandler.IsSingleFingerDragging || mTouchHandler.IsSingleFingerStationary)
    ...
    {
        if (!mTouchHandler.IsSingleFingerDragging && Mouse.current?.leftButton.isPressed != true
            || UnityRuntimeCompiledFacade.Instance.IsUnityUICurrentlySelected())
        {
            return;
        }

        var screenPosition = mTouchHandler.IsSingleFingerDragging
                                 ? mTouchHandler.SingleTouchPosition
                                 : Mouse.current.position.value;

        var cameraToPlaneRay = mMainCamera.ScreenPointToRay(screenPosition);
